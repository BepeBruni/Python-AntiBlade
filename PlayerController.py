...
class Player(Entity):
    def __init__(self, **kwargs):
        ...
        self.climbing = False
        self.grounded = True
        self.colliderSpeed = self.position+Vec3(self.velocity * time.dt * self.walk_speed * self.accel + self.dashSpeed * self.flip ,self.scale_y/2,0)
        ...
        camera.add_script(SmoothFollow(target=self, offset=[0,10,-175], speed=16))
    ...
    def update(self):
        if not boxcast(
            ...
            thickness=(self.scale_x*.99, self.scale_y*.99),
            debug=True,
            ).hit:
            if self.climbing == False:
                self.colliderSpeed = self.position+Vec3(self.velocity * time.dt * self.walk_speed * self.accel + self.dashSpeed * self.flip + 0.1,self.scale_y/2,0)
                self.x += self.velocity * time.dt * self.walk_speed * self.accel + self.dashSpeed * self.flip
            else:
                self.colliderSpeed =self.position+Vec3(0,self.scale_y/2,0)

        if not self.dashing:
            self.dashSpeed += -1/10
            self.dashSpeed = clamp(self.dashSpeed, 0, self.run_speed/2)
        ...
        #if not self.canDash:
        #    self.velocity = self.dashSpeed / 5
        ...
        left_ray = raycast(self.world_position+Vec3(-self.scale_x*.49,.1,0), self.down, distance=max(.15, self.air_time * self.gravity), ignore=self.ignore_list, traverse_target=self.traverse_target, debug=True)
        right_ray = raycast(self.world_position+Vec3(self.scale_x*.49,.1,0), self.down, distance=max(.15, self.air_time * self.gravity), ignore=self.ignore_list, traverse_target=self.traverse_target, debug=True)
        side_right_ray = raycast(self.world_position+Vec3(self.scale_x*.25,3,0), self.right, distance=max(2, 1), ignore=self.ignore_list, traverse_target=self.traverse_target, debug=True)
        Down_ray = raycast(self.world_position+Vec3(self.scale_x*.25,2.9,0), self.right, distance=max(2, 1), ignore=self.ignore_list, traverse_target=self.traverse_target, debug=True)

        # print(self.grounded)
        if not side_right_ray.hit and Down_ray:
            bufferJump = self.jump_height
            self.jump_height /= 2
            self.jump()
            self.jump_height = bufferJump
        if side_right_ray.hit:
            for things in side_right_ray.entities:
                 if things.name == "wall":
                     self.y += (abs(self.velocity)*time.dt * self.walk_speed * self.accel)/2
                     self.wallLand()
        else:
            self.climbing = False
        ...
        if not self.grounded and not self.jumping and not self.dashing:
        ...
        #camera.position = (self.x, 5+self.y, self.z-175)


    def input(self, key):
        if not held_keys['shift'] or (not held_keys['d'] and not held_keys['a']):
            self.accel = 1

        if key == 'shift' and self.canDash:
            self.accel += self.run_speed * 1
            self.dash()

        if held_keys['shift'] and (held_keys['d'] or held_keys['a']):
            self.accel += self.run_speed * 0.10
            self.accel = clamp(self.accel, 1, self.run_speed)

        if key == 'space':
            self.jump()

        if key == 'left mouse down':
            self.ignore_list.append(bullet(self.position + (0,5,0)))

        if key == 'd' or held_keys['d']:
            self.velocity = 1
    ...
    def dash(self):
        ...
        self.dashSpeed = self.run_speed/2
        invoke(self.dashPt2, delay = self.dashingTime)
    def dashPt2(self):
        self.dashing = False
        self.color = color.blue
        invoke(self.dashPt3, delay = self.dashCD)
    def dashPt3(self):
        self.canDash = True
        self.color = color.orange
