...
class Enemy(Entity):
    ...
    def update(self):
        if not boxcast(
            self.position+Vec3( float(self.velocity * self.walk_speed) * time.dt,self.scale_y/2,0),
            ...
            self.x += float(self.velocity * self.walk_speed) * time.dt
        else:
            self.velocity *= -1
    ...
    def walk(self):
        ...
        print("aqui: ", self.velocity)
        #if self.velocity != 0:
        invoke(self.walk2, delay = random.randint(1,4))
    def walk2(self):
        print("lá: ", self.velocity)
        self.canWalk = True
    def getHit(self):
        self.health -= 2.5
